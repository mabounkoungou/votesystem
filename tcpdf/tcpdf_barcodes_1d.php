...
class TCPDFBarcode {
      ...
      '200000',
		...
		switch(strtoupper((string) $type)) {
			...
			default: { // MODE AUTO
				...
				$numseq = [];
				preg_match_all('/([0-9]{4,})/', (string) $code, $numseq, PREG_OFFSET_CAPTURE);
				if (isset($numseq[1]) AND !empty($numseq[1])) {
					...
					foreach ($numseq[1] as $val) {
						...
						$sequence[] = ['C', substr((string) $code, $offset, $slen), $slen];
	...
	protected function get128ABsequence($code) {
		...
		$numseq = [];
		preg_match_all('/([\0-\31])/', (string) $code, $numseq, PREG_OFFSET_CAPTURE);
		if (isset($numseq[1]) AND !empty($numseq[1])) {
			$end_offset = 0;
			foreach ($numseq[1] as $val) {
				$offset = $val[1];
				if ($offset > $end_offset) {
					// B sequence
					$sequence[] = ['B', substr((string) $code, $end_offset, ($offset - $end_offset)), ($offset - $end_offset)];
				}
				// A sequence
				$slen = strlen($val[0]);
				$sequence[] = ['A', substr((string) $code, $offset, $slen), $slen];
				$end_offset = $offset + $slen;
			}
			if ($end_offset < $len) {
				$sequence[] = ['B', substr((string) $code, $end_offset), ($len - $end_offset)];
			}
		} else {
			// only B sequence
			$sequence[] = ['B', $code, $len];
	...
	protected function barcode_imb_pre($code) {
		if (!preg_match('/^[fadtFADT]{65}$/', (string) $code) == 1) {
			return false;
		}
	...
	public function dec_to_hex($number) {
		...
		while($number > 0) {
			if($number == 0) {
				...
				array_push($hex, strtoupper(dechex(bcmod((string) $number, '16'))));
	...
	public function hex_to_dec($hex) {
		...
		for($pos = ($len - 1); $pos >= 0; --$pos) {
			$dec = bcadd($dec, bcmul(hexdec((string) $hex[$pos]), $bitval));
	...
	protected function imb_crc11fcs($code_arr) {
		...
		$data = hexdec((string) $code_arr[0]) << 5;
		for ($bit = 2; $bit < 8; ++$bit) {
			if (($fcs ^ $data) & 0x400) {
				$fcs = ($fcs << 1) ^ $genpoly;
			} else {
		...
		for ($byte = 1; $byte < 13; ++$byte) {
			$data = hexdec((string) $code_arr[$byte]) << 3;
			for ($bit = 0; $bit < 8; ++$bit) {
				if (($fcs ^ $data) & 0x400) {
					$fcs = ($fcs << 1) ^ $genpoly;
				} else {
