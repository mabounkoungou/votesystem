...
class TCPDF {
  ...
  * @protected
  */
 protected $isunicode=true, $encoding='UTF-8', $diskcache=false, /**
 ...
 protected $pdfa_mode=false) {
		...
		TCPDF_FONTS::utf8Bidi([''], $this->CurrentFont, '', false, $this->isunicode);
		// set default font
	...
	protected function adjustCellPadding($brd=0) {
       ...
       default => $line_width / 2,
			...
			if ((str_contains($border,'T')) AND ($this->cell_padding['T'] < $adj)) {
				$this->cell_padding['T'] = $adj;
			}
			if ((str_contains($border,'R')) AND ($this->cell_padding['R'] < $adj)) {
				$this->cell_padding['R'] = $adj;
			}
			if ((str_contains($border,'B')) AND ($this->cell_padding['B'] < $adj)) {
				$this->cell_padding['B'] = $adj;
			}
			if ((str_contains($border,'L')) AND ($this->cell_padding['L'] < $adj)) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (($type != 'core') AND $missing_style) {
			...
			$name .= $styles[$bistyle];
			// artificial bold
			if (str_contains((string) $bistyle, 'B')) {
				if (isset($desc['StemV'])) {
					// from normal to bold
			...
			// artificial italic
			if (str_contains((string) $bistyle, 'I')) {
	...
	protected function getCellBorder($x, $y, $w, $h, $brd) {
		...
		foreach ($brd as $border => $style) {
			...
			if (strlen((string) $border) == 4) {
				...
				if (!str_contains((string) $border,'B')) { // LTR
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif (!str_contains((string) $border,'L')) { // TRB
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				} elseif (!str_contains((string) $border,'T')) { // RBL
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif (!str_contains((string) $border,'R')) { // BLT
				...
				if ((str_contains((string) $border,'L')) AND (str_contains((string) $border,'T'))) { // LT
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				} elseif ((str_contains((string) $border,'T')) AND (str_contains((string) $border,'R'))) { // TR
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif ((str_contains((string) $border,'R')) AND (str_contains((string) $border,'B'))) { // RB
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				} elseif ((str_contains((string) $border,'B')) AND (str_contains((string) $border,'L'))) { // BL
					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif ((str_contains((string) $border,'L')) AND (str_contains((string) $border,'R'))) { // LR
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif ((str_contains((string) $border,'T')) AND (str_contains((string) $border,'B'))) { // TB
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
					$s .= sprintf('%F %F m ', $xB, $yB);
				...
				if (str_contains((string) $border,'L')) { // L
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif (str_contains((string) $border,'T')) { // T
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				} elseif (str_contains((string) $border,'R')) { // R
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif (str_contains((string) $border,'B')) { // B
					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				}
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=[]) {
		...
		if ($file[0] === '@') {
			...
			if (($imsize = @getimagesize($file)) === FALSE) {
				...
				} elseif (!str_contains((string) $file, '__tcpdf_'.$this->file_id.'_img')) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			// get existing image data
			$info = $this->getImageBuffer($file);
			if (!str_contains((string) $file, '__tcpdf_'.$this->file_id.'_imgmask_')) {
		...
		} elseif (($ismask === false) AND ($imgmask === false) AND (!str_contains((string) $file, '__tcpdf_'.$this->file_id.'_imgmask_'))) {
		...
		if ($newimage) {
			...
			if ((method_exists('TCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				...
				if (($ismask === false) AND ($imgmask === false) AND (!str_contains((string) $file, '__tcpdf_'.$this->file_id.'_imgmask_'))
	...
	protected function getGDgamma($img, $c) {
		if (!isset($this->gdgammacache['#'.$c])) {
			...
			// correct gamma
			$this->gdgammacache['#'.$c] = (($this->gdgammacache['#'.$c] / 255) ** 2.2 * 255);
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		...
		$preserve = ['file_id', 'internal_encoding', 'state', 'bufferlen', 'buffer', 'cached_files', 'sign', 'signature_data', 'signature_max_length', 'byterange_string', 'tsa_timestamp', 'tsa_data'];
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=[], $fill_color=[]) {
		...
		if (!(!str_contains((string) $style, 'F')) AND !empty($fill_color)) {
	...
	public function Curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3, $style='', $line_style=[], $fill_color=[]) {
		...
		if (!(!str_contains((string) $style, 'F')) AND isset($fill_color)) {
			$this->SetFillColorArray($fill_color);
		}
		$op = TCPDF_STATIC::getPathPaintOperator($style);
		if ($line_style) {
	...
	public function Bookmark($txt, $level=0, $y=-1, $page='', $style='', $color=[0, 0, 0], $x=-1, $link='') {
		...
		$this->outlines[] = ['t' => $txt, 'l' => $level, 'x' => $x, 'y' => $y, 'p' => $page, 'f' => $fixed, 's' => strtoupper((string) $style), 'c' => $color, 'u' => $link];
	}
	...
	public function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
		if (str_contains((string) $colors, 'ALLSPOT')) {
			// expand spot colors
			$spot_colors = '';
	...
	protected function getCSSBorderDashStyle($style) {
		$dash = match (strtolower((string) $style)) {
      'none', 'hidden' => -1,
      'dotted' => 1,
      'dashed' => 3,
      default => 0,
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
      ...
      'x-large' => $refsize + 4,
      'xx-large' => $refsize + 6,
      'smaller' => $parent_size - 3,
      'larger' => $parent_size + 3,
      default => $this->getHTMLUnitToUnits($val, $parent_size, $defaultunit, true),
  };
		return $size;
	}
	...
	protected function getHtmlDomArray($html) {
		...
		while ($elkey < $maxel) {
         ...
         $dom[$key]['attribute'][strtolower((string) $name)] = $attr_array[2][$id];
          ...
          $dom[$key]['style'][strtolower((string) $name)] = trim($style_array[2][$id]);
      }
