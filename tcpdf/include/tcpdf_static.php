	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function replacePageNumAliases($page, $replace, $diff=0) {
		foreach ($replace as $rep) {
			foreach ($rep[3] as $a) {
				if (str_contains((string) $page, (string) $a)) {
	...
	public static function convertStringToHexString($s) {
		$bs = '';
		$chars = preg_split('//', (string) $s, -1, PREG_SPLIT_NO_EMPTY);
		foreach ($chars as $c) {
	...
	public static function encodeNameObject($name) {
		...
		for ($i = 0; $i < $length; ++$i) {
			$chr = $name[$i];
			if (preg_match('/[0-9a-zA-Z#_=-]/', (string) $chr) == 1) {
				$escname .= $chr;
			} else {
	...
	public static function getSVGTransformMatrix($attribute) {
		...
		$tm = [1, 0, 0, 1, 0, 0];
		$transform = [];
		if (preg_match_all('/(matrix|translate|scale|rotate|skewX|skewY)[\s]*\(([^\)]+)\)/si', (string) $attribute, $transform, PREG_SET_ORDER) > 0) {
...
// ====================================================================================================================
	...
	public static function pregSplit($pattern, $modifiers, $subject, $limit=NULL, $flags=NULL) {
		...
		if ((!str_contains((string) $modifiers, 'u')) OR (count(preg_split('//u', "\n\t", -1, PREG_SPLIT_NO_EMPTY)) == 2)) {
			return preg_split($pattern.$modifiers, (string) $subject, $limit, $flags);
		}
		// preg_split is bugged - try alternative solution
		$ret = [];
		while (($nl = strpos((string) $subject, "\n")) !== FALSE) {
			$ret = array_merge($ret, preg_split($pattern.$modifiers, substr((string) $subject, 0, $nl), $limit, $flags));
			$ret[] = "\n";
			$subject = substr((string) $subject, ($nl + 1));
		}
		if (strlen((string) $subject) > 0) {
			$ret = array_merge($ret, preg_split($pattern.$modifiers, (string) $subject, $limit, $flags));
		}
		return $ret;
	}
	...
	public static function fileGetContents($file) {
		...
		//
		$url = $file;
		if (preg_match('%^//%', (string) $url) && !empty($_SERVER['HTTP_HOST'])) {
		...
		if (isset($_SERVER['SCRIPT_URI'])
		    && !preg_match('%^(https?|ftp)://%', (string) $file)
		    && !preg_match('%^//%', (string) $file)
		) {
		...
		foreach ($alt as $path) {
			...
			if (!ini_get('allow_url_fopen')
				...
				&& preg_match('%^(https?|ftp)://%', (string) $path)
			) {
	 ...
	 * @public static
	 */
	public static function setPageBoxes($page, $type, $llx, $lly, $urx, $ury, $k, $points=false, $pagedim=[]) {
